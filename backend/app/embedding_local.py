...
def embed(texts: list[str], input_type: str | None = None) -> np.ndarray:
    # input_type is a Voyage-specific hint (query vs document); this model has
    # no equivalent concept, accepted-and-ignored so both providers share one call shape
    model = get_model()
